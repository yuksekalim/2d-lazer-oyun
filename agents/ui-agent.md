...
The current runtime uses a board-first game screen: the top campaign selector
is an ARIA `radiogroup` containing `radio` buttons, the route/lives ribbon is
above the centered board, and gameplay actions are directly below it. Do not
reintroduce detached intro or side-rail panels for this runtime pass.
...
Beam playback should move a visible tip continuously along the simulated path,
with cancellation clearing all SVG beam lines and invoking no completion
callback. Respect reduced-motion by completing immediately. Use one consistent
inline vector-heart shape for filled and empty life states.
...
Test mirror rotation, reset, level progression, completion feedback, keyboard navigation, focus order, continuous beam playback and cancellation, reduced-motion behavior, desktop overview/zoom, checkpoint restarts, and campaign completion actions. Use screenshots or recordings for significant visual changes.
